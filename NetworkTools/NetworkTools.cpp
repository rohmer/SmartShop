...
sSignalStrength NetworkTools::GetSignalStrength()
{
	sSignalStrength ret;
	ret.network = GetConnectedNetwork();
	if (ret.network.length()==0)
	{
		ret.signalStrength = -1;
		return ret;
	}
	if (InitiateNetworkScan())
	{
		usleep(500);
		std::vector<sWirelessNetwork> nets = GetScanResults();
		for (std::vector<sWirelessNetwork>::iterator it = nets.begin(); it != nets.end(); it++)
		{
			if (strcmp(it->ssid.c_str(), ret.network.c_str()) == 0)
			{
				if (it->signalStrength > -40)
				{
					ret.signalStrength = 4;
					return ret;
				}
				if (it->signalStrength > -65)
				{
					ret.signalStrength = 3;
					return ret;
				}
				if (it->signalStrength > -75)
				{
					ret.signalStrength = 2;
					return ret;
				}
				if (it->signalStrength > -83)
				{
					ret.signalStrength = 1;
					return ret;
				}
				ret.signalStrength = 0;
				return ret;
			}
		}
	}
